...
window.H = Object.assign(
  ...
  mutabakat,
  {
    ...
    accountIconPointerDown: cloudBootstrap.accountIconPointerDown,
    accountIconPointerUp: cloudBootstrap.accountIconPointerUp,
    accountIconPointerCancel: cloudBootstrap.accountIconPointerCancel,
...
async function runRoute() {
  ...
  for (const route of ROUTES) {
    ...
    if (m) {
      ...
      ui.setFabAction(null); // reset before each screen renders — see ui.js
      try {
...
// -------- FAB / quick add --------------------------------------------------------
// §nav-redesign: each screen registers its own contextual action via
// ui.setFabAction() (see ui.js). No screen has registered one (or it left
// the app's most general-purpose screen, Ana Sayfa, in place) → fall back
// to the original broad quick-add menu.
document.getElementById('fabBtn').addEventListener('click', () => {
  const action = ui.getFabAction();
  if (action) action(); else forms.openQuickAddMenu();
});
