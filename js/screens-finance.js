...
function financeTopbar(title, rightHtml) {
  setTopbar(`<div class="left"><div><h1>Finans</h1><div class="sub">${title}</div></div></div>${rightHtml ? `<div class="right">${rightHtml}</div>` : ''}`);
}
...
// §nav-redesign: Finans → Müşteri Detayı was merged into the Müşteri Detayı
// screen (screens.js renderClientDetail) — one client, one full page
// (structure + money + history) no matter which tab you arrived from. This
// route is kept alive as a redirect only, so old links/bookmarks/back-button
// history still land somewhere correct instead of a dead end.
...
async function vendorHierarchyBody() {
  ...
  html += `<div class="section-title">Mecra Türleri <span class="link" onclick="H.openMecraQuickAddMenu()">+ Ekle</span></div>`;
...
export async function renderMediaTypeDetail({ mediaType }) {
  ...
  setTopbar(`
    <div class="left">
      ...
      <div><h1>${escapeHtml(typeName)}</h1><div class="sub">Mecralar</div></div>
    </div>
  `);
  setContent(`<div class="list-loading">Yükleniyor…</div>`);
...
// FINANS → ÇEKLER
// ============================================================================
// §cheque-autopay: the old two-axis filter (Elimde/Kullanılan segment ×
// 6-way status pill) needed two correct taps to reach any useful view. It's
// replaced by one search box plus a single always-visible priority order —
// Vadesi Gelen → Elimde Bekliyor → Verilmiş/Kullanılmış → Sonuçlanmış — so opening
// the page IS the filter: most-urgent-first, no clicking required. Search
// still narrows across all of them when you're hunting one specific cheque.
let chequeSearch = '';
let chequeShowResolved = false;

function chequeMatchesSearch(c, q) {
  if (!q) return true;
  const hay = [c.counterpartyName, c.campaignName, c.bank, c.chequeNumber, chequeUsedFragment(c)]
    .filter(Boolean).join(' ').toLocaleLowerCase('tr-TR');
  return hay.includes(q.toLocaleLowerCase('tr-TR'));
}

function chequeGroupSectionHtml(title, list, { urgent } = {}) {
  if (!list.length) return '';
  // section-title is a flex row (space-between) — title+count wrapped in one
  // span so they stay glued together on the left instead of getting spread
  // apart across the row like a title/link pair would.
  return `<div class="section-title"><span>${title} <span class="chip neutral">${list.length}</span></span></div>` +
    list.map((c) => chequeRowHtml(c, { onDelete: (cc) => `H.deleteChequeRecord('${cc.id}')`, urgent })).join('');
}
...
export async function renderCheques() {
  ...
  const withGroup = all.map((c) => ({ c, group: calc.chequeGroup(c, today) }));
  const elimde = withGroup.filter((x) => x.group === 'elimde').map((x) => x.c);
  const acil = withGroup.filter((x) => x.group === 'acil').map((x) => x.c);
  const verilmis = withGroup.filter((x) => x.group === 'verilmis').map((x) => x.c);
  const sonuclandi = withGroup.filter((x) => x.group === 'sonuclandi').map((x) => x.c);

  let html = tabStrip('cheques');
  html += `<div class="search-box"><input id="chequeSearchInput" placeholder="Müşteri, yüklenici, banka, çek no ara…" value="${escapeHtml(chequeSearch)}"></div>`;

  html += `
    <div class="summary-strip">
      <div class="si"><div class="label">Elimde</div><div class="value">${fmtN(elimde.reduce((s, c) => s + (Number(c.amount) || 0), 0))}</div></div>
      <div class="si red"><div class="label">Vadesi Gelen (Bugün)</div><div class="value">${fmtN(acil.reduce((s, c) => s + (Number(c.amount) || 0), 0))}</div></div>
      <div class="si"><div class="label">Verilmiş</div><div class="value">${fmtN(verilmis.reduce((s, c) => s + (Number(c.amount) || 0), 0))}</div></div>
    </div>
  `;

  if (chequeSearch.trim()) {
    // Arama aktifken grup ayrımı bir kenara bırakılır — "aradığımı bul" tek
    // düz liste, vade tarihine göre sıralı.
    const q = chequeSearch.trim();
    const matches = all.filter((c) => chequeMatchesSearch(c, q)).sort((a, b) => (a.dueDate || '').localeCompare(b.dueDate || ''));
    html += `<div class="section-title">Arama Sonucu <span class="chip neutral">${matches.length}</span></div>`;
    if (matches.length === 0) {
      html += emptyState(icon('search', { size: 32 }), 'Sonuç bulunamadı', 'Farklı bir isim ya da banka/çek no dene.');
    } else {
      html += matches.map((c) => chequeRowHtml(c, { onDelete: (cc) => `H.deleteChequeRecord('${cc.id}')`, urgent: calc.chequeGroup(c, today) === 'acil' })).join('');
    }
  } else if (all.length === 0) {
    html += emptyState(icon('receipt', { size: 32 }), 'Henüz çek yok', 'Bir müşteriden çek tahsil ettiğinde burada görünecek.');
  } else {
    acil.sort((a, b) => (a.dueDate || '').localeCompare(b.dueDate || ''));
    elimde.sort((a, b) => (a.dueDate || '').localeCompare(b.dueDate || ''));
    verilmis.sort((a, b) => (a.dueDate || '').localeCompare(b.dueDate || ''));
    sonuclandi.sort((a, b) => (b.dueDate || '').localeCompare(a.dueDate || ''));

    html += chequeGroupSectionHtml('Vadesi Gelen (Bugün)', acil, { urgent: true });
    html += chequeGroupSectionHtml('Elimde Bekliyor', elimde);
    html += chequeGroupSectionHtml('Verilmiş / Kullanılmış', verilmis);

    // Sonuçlanmış (Ödendi/Karşılıksız/İptal — otomatik ya da elle) zamanla
    // kabaracak bir arşiv; varsayılan kapalı, istenince tek tıkla açılıyor.
    html += `<div class="section-title"><span>Sonuçlanmış <span class="chip neutral">${sonuclandi.length}</span></span>${sonuclandi.length ? `<span class="link" onclick="H.toggleChequeResolved()">${chequeShowResolved ? 'Gizle' : 'Göster'}</span>` : ''}</div>`;
    if (chequeShowResolved) {
      html += sonuclandi.length
        ? sonuclandi.map((c) => chequeRowHtml(c, { onDelete: (cc) => `H.deleteChequeRecord('${cc.id}')` })).join('')
        : `<p class="hint">Henüz sonuçlanmış çek yok.</p>`;
    }
  }
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      chequeSearch = e.target.value;
      renderCheques();
      setTimeout(() => {
        const el = document.getElementById('chequeSearchInput');
...
export function toggleChequeResolved() { chequeShowResolved = !chequeShowResolved; renderCheques(); }
  ...
  let html = `
    ...
    <div class="detail-card">
      ...
      <div class="status-select-row">
        ${calc.CHEQUE_MANUAL_STATUSES.map((s) => {
          const isManual = cheque.status === s;
          const isAuto = !cheque.status && s === 'Ödendi' && status === 'Ödendi';
          return `<button class="status-opt ${isManual ? 'active' : (isAuto ? 'auto' : '')}" onclick="H.setChequeStatus('${cheque.id}','${s}')">${s}${isAuto ? ' (otomatik)' : ''}</button>`;
        }).join('')}
        ${cheque.status ? `<button class="status-opt" onclick="H.setChequeStatus('${cheque.id}',null)">Sıfırla</button>` : ''}
      </div>
      <p class="hint" style="margin-top:8px;">Vade tarihi geldiğinde çek "Vadesi Gelen" gösterilir; ertesi güne kadar sen Karşılıksız/İptal işaretlemezsen otomatik olarak "Ödendi" sayılır. İstisna varsa buradan elle işaretle, otomatik işareti geçersiz kılar.</p>
    </div>
